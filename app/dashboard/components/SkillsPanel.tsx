...
const empty: SkillInput = { name: '', category: '', level: 50, iconUrl: '' }
...
function SkillForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <FormActions loading={loading} onCancel={onCancel} submitLabel="save skill" />
    </form>
  )
}
...
export default function SkillsPanel() {
  ...
  const reload = () => fetchSkills().then(setItems).finally(() => setLoading(false))
  useEffect(() => { reload() }, [])
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {categories.map((cat) => (
            <div key={cat}>
              <p className="text-green-800 text-xs font-mono tracking-widest uppercase mb-2">{cat}</p>
                  ...
                  .map((s) => (
                    ...
                    >
                      <div className="flex items-center gap-4 min-w-0 flex-1">
                        <span className="text-gray-300 text-sm w-28 truncate shrink-0">{s.name}</span>
                        ...
                        <span className="text-gray-600 text-xs font-mono w-10 text-right shrink-0">{s.level}%</span>
                      </div>
      ...
      {modal && (
        <Modal
          title={modal === 'create' ? 'New Skill' : `Edit: ${(modal as Skill).name}`}
